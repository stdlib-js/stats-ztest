...
function ztest( x, sigma, options ) {
	...
	if ( !isTypedArrayLike( x ) && !isNumberArray( x ) ) {
		throw new TypeError( format( '0ft8j', x ) );
	}
	if ( !isPositiveNumber( sigma ) ) {
		throw new TypeError( format( '0ft7T', sigma ) );
	}
	len = x.length;
	opts = {};
	if ( options ) {
	...
	if ( len < 2 ) {
		throw new Error( format( '0ftAn', x ) );
	}
	...
	case 'less':
		...
		break;
	default:
		throw new Error( format( '0ft3t', 'alternative', [ 'two-sided', 'less', 'greater' ].join( '", "' ), alt ) );
	}
	out = {};
