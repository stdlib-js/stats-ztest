...
function ztest( x, sigma, options ) {
	...
	if ( !isPositiveNumber( sigma ) ) {
		throw new TypeError( format( '1Lz72', sigma ) );
	}
	len = x.length;
	opts = {};
	if ( options ) {
	...
	if ( alpha < 0.0 || alpha > 1.0 ) {
		throw new RangeError( format( '1Lz8V', 'alpha', alpha ) );
	}
	if ( len < 2 ) {
		throw new Error( format( '1LzA2', x ) );
	}
	...
	case 'less':
		...
		break;
	default:
		throw new Error( format( '1Lz4S', 'alternative', [ 'two-sided', 'less', 'greater' ].join( '", "' ), alt ) );
	}
	out = {};
